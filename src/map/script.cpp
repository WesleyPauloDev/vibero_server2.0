...
/*========================================================
 * Retorna o b�nus de drop para um item espec�fico de um mob,
 * considerando os b�nus do jogador que est� executando o script.
 * Uso em script: get_drop_bonus(<mob_id>, <item_id>);
 *-------------------------------------------------------*/
BUILDIN_FUNC(get_drop_bonus)
{
    map_session_data* sd;
    // Obt�m o jogador atual (charid tipo 4 = script executor)
    if ( !script_charid2sd(4, sd) ) {
    ...
    for (const auto& drop : mob->dropitem) {
        if (drop->nameid == item_id) {
            ...
            script_pushint(st, final_rate); // em cent�simos de %
            return SCRIPT_CMD_SUCCESS;
        }
    }
...
// Fun��o para script: define b�nus de drop permanente
...
BUILDIN_FUNC(set_bonus_hpsp)
{
    ...
    int val_hp = script_getnum(st, 3); // b�nus de HP
    int val_sp = script_hasdata(st, 4) ? script_getnum(st, 4) : val_hp; // b�nus de SP (opcional)

    // Atribui os valores separadamente
    sd->bonus_hp = val_hp;
    sd->bonus_sp = val_sp;

    // For�a recalcular status completo
	status_calc_pc(sd, SCO_FORCE);
