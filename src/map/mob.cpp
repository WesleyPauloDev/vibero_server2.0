...
int32 mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int32 base_rate, int32 drop_modifier, mob_data* md)
{
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			int luk_bonus1 = 0, luk_bonus2 = 0;
			if (battle_config.drops_by_luk)
				luk_bonus1 = status_get_luk(src) * battle_config.drops_by_luk / 100;
			if (battle_config.drops_by_luk2)
				luk_bonus2 = (int32)(0.5 + drop_rate * status_get_luk(src) * battle_config.drops_by_luk2 / 10000.);
			...
			if (sd->sc.getSCE(SC_ITEMBOOST))
				itemboost = sd->sc.getSCE(SC_ITEMBOOST)->val1;
			int itemboost2 = 0;
			if (sd->sc.getSCE(SC_PERIOD_RECEIVEITEM_2ND))
				itemboost2 = sd->sc.getSCE(SC_PERIOD_RECEIVEITEM_2ND)->val1;

			int vip_bonus = 0;
			if (pc_isvip(sd))
				vip_bonus = battle_config.vip_drop_increase;
			...
			drop_rate_bonus += pk_bonus + class_bonus + race_bonus + itemboost + itemboost2 + vip_bonus;

			int32 cap = pc_isvip(sd) ? battle_config.drop_rate_cap_vip : battle_config.drop_rate_cap;

			// Aplica multiplicador final
			drop_rate = (int32)(0.5 + drop_rate * drop_rate_bonus / 100.);

			// Aplica limite de cap
