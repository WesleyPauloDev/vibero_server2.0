...
// Refresh the client's screen, getting rid of visual effects and stale objects.
void clif_refresh(map_session_data *sd)
{
	nullpo_retv(sd);

	// A same-map move is required for a real client-side refresh. Rebuilding the
	// visible object list alone does not clear the client's map/effect state.
...
void clif_parse_OpenVending(int32 fd, map_session_data* sd){
	...
	// Criar novo nome da loja sem risco de mem�ria
...
void clif_display_pinfo( map_session_data& sd ){
	...
	for( int32 i = PINFO_BASIC; i < PINFO_MAX; i++ ){
		...
		if (pc_isvip(&sd))
			p->total_death = battle_config.vip_exp_penalty_base * 10;
		else
			p->total_death = battle_config.death_penalty_base * 10;

		if (pc_isvip(&sd))
			p->total_drop += p->details[i].drop * 12 / 26;
		else
