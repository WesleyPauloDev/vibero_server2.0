...
int32 pc_equippoint_sub(map_session_data *sd,struct item_data* id){
	...
	if(id->subtype == W_DAGGER	|| id->subtype == W_1HSWORD || id->subtype == W_1HAXE) {
		if(pc_checkskill(sd,AS_LEFT) > 0 || (sd->class_&MAPID_UPPERMASK) == MAPID_ASSASSIN || (sd->class_&MAPID_UPPERMASK) == MAPID_NINJA || (sd->class_&MAPID_UPPERMASK) == MAPID_KAGEROUOBORO) { //Ninja, Kagerou and Oboro can dual wield daggers. [Rytech]
			if (ep == EQP_WEAPON)
				return EQP_ARMS;
...
void pc_set_reborn_exp(map_session_data* sd, int val)
{
    ...
    // for�a recalcular status (opcional, caso queira que j� reflita em c�lculos dependentes)
    status_calc_pc(sd, SCO_NONE);
}
...
void pc_gainexp(map_session_data *sd, struct block_list *src, t_exp base_exp, t_exp job_exp, uint8 exp_flag)
{
	...
	if (!(exp_flag&2)) {
		...
		// --- B�nus Reborn de EXP (pode ser negativo) ---
		if (sd->reborn_exp != 0) {
...
int32 pc_dead(map_session_data *sd,struct block_list *src)
{
	...
	if(sd->status.pet_id > 0 && sd->pd) {
		...
		if( battle_config.pet_owner_die_intimacy_loss && !mapdata->getMapFlag(MF_NOEXPPENALTY) ) {
...
bool pc_equipitem(map_session_data *sd,int16 n,int32 req_pos,bool equipswitch)
{
	...
	if(pos == EQP_ACC) { //Accessories should only go in one of the two.
		...
		if (pos == EQP_ARMS || (pos == EQP_HAND_R && equip_index[EQI_HAND_R] >= 0 && equip_index[EQI_HAND_L] < 0)) // Pick left hand when right hand is already occupied.
#ifdef RENEWAL
