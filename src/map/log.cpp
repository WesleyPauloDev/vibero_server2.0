...
static constexpr const char* ENCHANTGRADE_LOG_TABLE = "enchantgrade_log";
...
static const char* log_enchantgrade_result_name( e_log_enchantgrade_result result ){
	switch( result ){
		case LOG_ENCHANTGRADE_SUCCESS:
			return "success";
		case LOG_ENCHANTGRADE_FAILURE:
			return "failure";
		case LOG_ENCHANTGRADE_BREAK:
			return "break";
		case LOG_ENCHANTGRADE_DOWNGRADE:
			return "downgrade";
	}
...
void log_enchantgrade_event( map_session_data* sd, e_log_enchantgrade_result result, struct item* itm, uint8 grade_before, uint8 grade_after, uint8 refine_before, uint8 refine_after, t_itemid material_item_id, uint16 material_amount, t_itemid catalyst_item_id, uint16 catalyst_amount, uint32 zeny, uint16 chance ){
	...
	StringBuf_Printf( &buf,
		LOG_QUERY " INTO `%s` (`time`, `account_id`, `char_id`, `char_name`, `map`, `result`, `item_id`, `item_name`, `unique_id`, `grade_before`, `grade_after`, `refine_before`, `refine_after`, `material_item_id`, `material_amount`, `catalyst_item_id`, `catalyst_amount`, `zeny`, `chance`, `bound`, `attribute`, `identify`",
		ENCHANTGRADE_LOG_TABLE );
	...
	StringBuf_Printf( &buf,
		") VALUES (NOW(), '%d', '%d', '%s', '%s', '%s', '%u', '%s', '%" PRIu64 "', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%d', '%d', '%d'",
		sd->status.account_id,
		sd->status.char_id,
		esc_char_name,
		mapindex_id2name( sd->mapindex ),
		log_enchantgrade_result_name( result ),
		itm->nameid,
		esc_item_name,
		itm->unique_id,
		grade_before,
