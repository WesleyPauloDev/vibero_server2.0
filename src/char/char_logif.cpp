...
int32 chlogif_parse_reqaccdata(int32 fd){
	...
	if( u_fd < fd_max )
	{
		...
		if( sd->char_slots > MAX_CHARS ) {
			ShowError("Account '%d' `character_slots` column (%d) is higher than supported MAX_CHARS (%d), update MAX_CHARS in mmo.hpp! capping to MAX_CHARS...\n",sd->account_id,sd->char_slots,MAX_CHARS);
